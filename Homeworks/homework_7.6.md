...
### Ответ
1. resource и data_source перечислены в файле terraform-provider-aws/aws/provider.go:  
...
2. * параметр name конфликтует с параметром name_prefix:  
   ...
   * имя должно подчиняться регулярному выражению `^[0-9A-Za-z-_]+(\.fifo)?$` (разрешены только буквенно-цифровые символы и дефисы):  
